...
def hex_to_bin(arr):
    ...
    bin_arr = []
    for hex in arr:
        if hex:         #빈 문자열이 존재하기 때문에 값이 있는것만 2진수로 변환
...
def get_width(arr):
    ...
    for row in range(len(arr)):
        count = 0
        before = '0'  # 이전 값을 저장(다음 값과 비교하기 위해)
        change = 0  # 숫자가 0->1, 1->0으로 몇번 바뀌었는지 확인용 변수
...
def scanner(bin_arr, width_list):
    ...
    for row in range(len(bin_arr)):
        while bin_arr[row]:
            ...
            #자른 코드를 검증하는 시간
            pw_list = []
            for i in range(8): # 8자리의 암호이기 대문에 8번 반복
...
for tc in range(1, T+1):
    ...
    arr = list(set([input().strip('0') for _ in range(N)]))
    # 16진수를 2진수로 변환 완료
    bin_arr = hex_to_bin(arr)
    # 과연 몇 배수로 이루어져 있는지 확인!!
    # 마지막 숫자 정보를 읽어서 7의 몇 배수인지 확인, 마지막 지점정보도 같이 가져오자
    width_list = get_width(bin_arr)     #배수정보와 마지작 지점이 리스트로 전달.
